...
struct mtat_page *alloc_and_init_mtat_page(struct page *page)
{
	...
	m_page->is_promoted = false;
	m_page->accesses_after_promo = 0;
...
static void pebs_sample(uint64_t pfn, size_t config) 
{
	...
	// 접근횟수 카운팅
	m_page->accesses++;
	if (m_page->is_promoted)
		m_page->accesses_after_promo++;
		...
		struct mtat_page *m_page)
{
	...
	if (m_page->nid == FASTMEM) {
		/* Promotion */
		m_page->is_promoted = true;
		m_page->accesses_after_promo = 0;
		app->nr_promotions++;
	}
		...
		struct list_head *to, int target_nr)
{
	...
	list_for_each_entry_safe(m_page, tmp, &from->list, list) {
		...
		app = &apps[m_page->apps_idx];
		if (m_page->is_promoted) {
			/* Demotion */
			app->nr_demotions[hg_get_idx(m_page->accesses_after_promo)]++;
			m_page->is_promoted = false;
			m_page->accesses_after_promo = 0;
		}
...
static ssize_t nr_promotions_show(struct kobject *kobj, struct kobj_attribute *attr,
		char *buf)
{
	struct mtat_sysfs_app_dir *app_dir = container_of(kobj, struct mtat_sysfs_app_dir, kobj);
	struct app_struct *app = &apps[app_dir->app_idx];
...
static ssize_t nr_demotions_show(struct kobject *kobj, struct kobj_attribute *attr,
		char *buf)
{
	struct mtat_sysfs_app_dir *app_dir =container_of(kobj, struct mtat_sysfs_app_dir, kobj);
	struct app_struct *app = &apps[app_dir->app_idx];
	...
	for (i = 0; i < 16; i++) {
		ret = snprintf(buf + len, buf_size - len, "%llu ", app->nr_demotions[i]);
...
static struct kobj_attribute nr_promotions_attr = __ATTR_RO_MODE(nr_promotions, 0400);
static struct kobj_attribute nr_demotions_attr = __ATTR_RO_MODE(nr_demotions, 0400);
...
static struct attribute *mtat_sysfs_app_dir_attrs[] = {
	...
	&histogram_attr.attr,
	&nr_promotions_attr.attr,
	&nr_demotions_attr.attr,
	NULL,
};
