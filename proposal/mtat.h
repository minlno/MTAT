...
struct app_struct {
	...
	uint64_t nr_promotions; // promotion된 페이지 수
	/*
	 * promotion된 이후에 demotion된 페이지 수. 
	 * promotion ~ demotion 사이의 기간동안 접근된 횟수에 따라 서로다른 인덱스에 카운팅됨.
	 * N이 2^(i)보다 작으면 nr_demotions[i]에 카운팅.
	 * ex)
	 *   N = 0 -> nr_demotions[0]++;
	 *   N = 1 -> nr_demotions[1]++;
	 *   N = 2 or 3 -> nr_demotions[2]++;
	 */
	uint64_t nr_demotions[16];
};
